    ...
    <div class="section-body">
        <div class="card shadow">
            <div class="card-header bg-primary text-white py-1">
                <h4><i class="fas fa-fw fa-list"></i> Edit Ujian</h4>
                <div class="ml-auto">
                    <a href="{{ route('exam.index') }}" class="btn btn-reka"><i class="fas fa-arrow-left"></i> Kembali</a>
                </div>
            </div>
                ...
                <form action="{{ route('exam.update', $exam->id) }}" method="post">
                    ...
                    <div class="form-group row">
                        <div class="col-md-6">
                            ...
                            <select name="exam_type" id="exam_type" class="form-control custom-select @error('exam_type') is-invalid @enderror" required>
                                ...
                                <option value="UKOM" {{ (old('exam_type') ?? $exam->exam_type) == 'UKOM' ? 'selected' : '' }}>UKOM</option>
                                <option value="TKA" {{ (old('exam_type') ?? $exam->exam_type) == 'TKA' ? 'selected' : '' }}>TKA</option>
                                <option value="ANBK" {{ (old('exam_type') ?? $exam->exam_type) == 'ANBK' ? 'selected' : '' }}>ANBK</option>
                            </select>
                            @error('exam_type')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="exam_name" id="exam_name" class="form-control @error('exam_name') is-invalid @enderror" value="{{ old('exam_name') ?? $exam->exam_name }}" placeholder="Masukkan nama ujian" required>
                            @error('exam_name')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="form-group row">
                        <div class="col-md-12">
                            ...
                            <textarea name="exam_description" id="texteditor">{{ old('exam_description') ?? $exam->exam_description }}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        <div class="col-md-6">
                            ...
                            <select name="exam_status" id="exam_status" class="form-control custom-select @error('exam_status') is-invalid @enderror" required>
                                ...
                                <option value="Active" {{ (old('exam_status') ?? $exam->exam_status) == 'Active' ? 'selected' : '' }}>Aktif</option>
                                <option value="Inactive" {{ (old('exam_status') ?? $exam->exam_status) == 'Inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('exam_status')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
